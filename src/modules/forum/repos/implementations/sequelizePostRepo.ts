...
export class PostRepo implements IPostRepo {
    ...
    public async getNumberOfFirstLevelComments(postId: PostId | string): Promise<number> {
        postId = postId instanceof PostId
            ? (<PostId>postId).id.toString()
            : postId;
    ...
    public async getRecentPosts(offset?: number): Promise<PostDetails[]> {
        ...
        for (const post of posts) {
            post.total_num_comments = promises.push(this.getNumberOfFirstLevelComments(post.post_id)
                .then(count => post.total_num_comments = count));
        }
